const APIKEY = "OCLB4FJ1FO6YUQE4";

document.getElementById("tickerForm").onsubmit = function () {
  let ticker = "";
...
async function fetchText(ticker) {
  ...
  let feed = data["Time Series (Daily)"];
  let results = [];
  for (let i in feed) {
    let buildObject = {};
    if (feed[i]) {
      buildObject.date = new Date(i);
      buildObject.open = parseFloat(feed[i]["1. open"]);
      buildObject.high = parseFloat(feed[i]["2. high"]);
      buildObject.low = parseFloat(feed[i]["3. low"]);
      buildObject.close = parseFloat(feed[i]["4. close"]);
      buildObject.volume = parseFloat(feed[i]["5. volume"]);
      results.unshift(buildObject);
    }
  }
  initialiseChart(results);
}
